...
class TestAutonomousCampaign:
            ...
            mock_execute.side_effect = [
                {
                    ...
                    "parameters": {"temperature": 150 + i*10, "concentration": 1.0, "time": 12},
            ...
            mock_execute.side_effect = [
                {
                    ...
                    "results": {
                        "band_gap": 1.4 + (i-5)*0.02,  # Trade-off: efficiency vs band_gap
    ...
    def test_campaign_with_failures(self, mock_lab_setup):
        ...
        with patch.object(lab, '_execute_experiment') as mock_execute:
            ...
            for i in range(8):
                if i in [2, 5]:  # Simulate failures
                    results.append({
                        "experiment_id": f"exp_{i:03d}",
                        "parameters": {"temperature": 150 + i*10, "concentration": 1.0},
                        "results": {},
                        "status": "failed",
        ...
        failed_experiments = [exp for exp in campaign.experiments if exp["status"] == "failed"]
    ...
    def test_resource_constraints(self, mock_lab_setup):
        ...
        with patch.object(lab, '_execute_experiment', side_effect=mock_execute_with_constraints):
            campaign = lab.run_campaign(
                objective=objective,
                param_space=param_space,
                initial_samples=2,
        ...
        assert any("resource" in exp.get("error", "").lower() for exp in campaign.experiments 
