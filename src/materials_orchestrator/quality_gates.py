...
class QualityGateRunner:
    ...
    def register_test_suite(self, suite_name: str, test_functions: List[Callable]):
        ...
        logger.info(
            f"Registered test suite '{suite_name}' with {len(test_functions)} tests"
        )

    def run_quality_gates(
        self, suites: Optional[List[str]] = None, fail_fast: bool = False
    ) -> QualityGateResult:
        ...
        for suite_name in suites_to_run:
            ...
            if fail_fast:
                ...
                if critical_failures:
                    logger.error(
                        f"Critical test failure in {suite_name}, stopping execution"
                    )
                    break
        ...
        skipped_tests = sum(
            1 for r in all_test_results if r.status == TestStatus.SKIPPED
        )
    ...
    def _run_test_suite(
        self, suite_name: str, fail_fast: bool = False
    ) -> List[TestResult]:
        ...
        for test_func in test_functions:
            try:
                ...
                if (
                    ...
                    and test_result.severity == TestSeverity.CRITICAL
                ):
                    break
    ...
    def _determine_overall_status(
        self, test_results: List[TestResult], success_rate: float
    ) -> TestStatus:
        ...
        if (
            critical_failures
            and len(critical_failures)
            > self.quality_standards["critical_test_failure_tolerance"]
        ):
    ...
    def _test_autonomous_lab_creation(self) -> TestResult:
        ...
        try:
            ...
            if not hasattr(lab, "status"):
                return TestResult(
                    "autonomous_lab_creation",
                    TestStatus.FAILED,
                    TestSeverity.CRITICAL,
                    0,
                    "Lab missing status attribute",
                )
            ...
            return TestResult(
                ...
                TestSeverity.CRITICAL,
                0,
                "Autonomous lab creation successful",
            )
            ...
            return TestResult(
                ...
                TestSeverity.CRITICAL,
                0,
                f"Exception during lab creation: {e}",
            )
    ...
    def _test_materials_objective_validation(self) -> TestResult:
        ...
        try:
            ...
            if not obj.target_property == "band_gap":
                return TestResult(
                    "materials_objective_validation",
                    TestStatus.FAILED,
                    TestSeverity.HIGH,
                    0,
                    "Target property not set correctly",
                )

            # Test invalid range
            try:
                invalid_obj = MaterialsObjective(
                    "test", (2.0, 1.0), "target"
                )  # Invalid range
                return TestResult(
                    "materials_objective_validation",
                    ...
                    TestSeverity.MEDIUM,
                    0,
                    "Invalid range validation failed",
                )
            except ValueError:
                pass  # Expected
            ...
            return TestResult(
                ...
                TestSeverity.HIGH,
                0,
                "Materials objective validation successful",
            )
            ...
            return TestResult(
                ...
                TestSeverity.HIGH,
                0,
                f"Exception during objective validation: {e}",
            )
    ...
    def _test_experiment_execution(self) -> TestResult:
        ...
        try:
            ...
            test_params = {"temperature": 150.0, "concentration": 1.0, "time": 3.0}
            ...
            if experiment.status == "completed" and not experiment.results:
                return TestResult(
                    ...
                    TestSeverity.HIGH,
                    0,
                    "Completed experiment has no results",
                )
            ...
            return TestResult(
                ...
                TestSeverity.CRITICAL,
                0,
                f"Exception during experiment execution: {e}",
            )
    ...
    def _test_campaign_execution(self) -> TestResult:
        ...
        try:
            ...
            campaign = lab.run_campaign(
                objective, param_space, initial_samples=3, max_experiments=5
            )
            ...
            if campaign.total_experiments == 0:
                return TestResult(
                    ...
                    TestSeverity.CRITICAL,
                    0,
                    "Campaign executed zero experiments",
                )
            ...
            if campaign.success_rate == 0:
                return TestResult(
                    ...
                    TestSeverity.HIGH,
                    0,
                    "Campaign had zero success rate",
                )
            ...
            return TestResult(
                ...
                TestSeverity.CRITICAL,
                0,
                f"Campaign executed {campaign.total_experiments} experiments",
            )
            ...
            return TestResult(
                ...
                TestSeverity.CRITICAL,
                0,
                f"Exception during campaign execution: {e}",
            )
    ...
    def _test_result_validation(self) -> TestResult:
        ...
        try:
            ...
            if experiment.status == "completed":
                ...
                if not isinstance(experiment.results, dict):
                    return TestResult(
                        ...
                        TestSeverity.HIGH,
                        0,
                        "Results not in dictionary format",
                    )
                ...
                for key, value in experiment.results.items():
                    if not isinstance(value, (int, float)):
                        return TestResult(
                            ...
                            TestSeverity.MEDIUM,
                            0,
                            f"Non-numeric result value: {key}={value}",
                        )
            ...
            return TestResult(
                ...
                TestSeverity.HIGH,
                0,
                f"Exception during result validation: {e}",
            )

    def _test_input_validation(self) -> TestResult:
        """Test input validation functionality."""
        try:
            from materials_orchestrator.security_enhanced import (
            ...
            is_valid, error = security_manager.validate_request(
                valid_input, "test", "test_resource"
            )
            ...
            return TestResult(
                ...
                TestSeverity.HIGH,
                0,
                f"Exception during input validation test: {e}",
            )
    ...
    def _test_malicious_input_blocking(self) -> TestResult:
        ...
        try:
            ...
            for malicious_input in malicious_inputs:
                is_valid, error = security_manager.validate_request(
                    malicious_input, "test", "test"
                )
                if is_valid:
                    return TestResult(
                        "malicious_input_blocking",
                        TestStatus.FAILED,
                        TestSeverity.CRITICAL,
                        0,
            ...
            return TestResult(
                ...
                TestSeverity.CRITICAL,
                0,
                "Malicious input blocking successful",
            )
            ...
            return TestResult(
                ...
                TestSeverity.CRITICAL,
                0,
                f"Exception during malicious input test: {e}",
            )
    ...
    def _test_parameter_sanitization(self) -> TestResult:
        ...
        try:
            ...
            for key in clean_params.keys():
                if "<" in key or ";" in key or "'" in key:
                    return TestResult(
                        ...
                        TestSeverity.HIGH,
                        0,
                        f"Dangerous characters not sanitized: {key}",
                    )
            ...
            return TestResult(
                ...
                TestSeverity.HIGH,
                0,
                "Parameter sanitization successful",
            )
            ...
            return TestResult(
                ...
                TestSeverity.HIGH,
                0,
                f"Exception during parameter sanitization test: {e}",
            )
        ...
        try:
            ...
            if success_count == 10:
                ...
                return TestResult(
                    ...
                    TestSeverity.MEDIUM,
                    0,
                    f"Rate limiting test passed ({success_count}/10 requests allowed)",
                )
            ...
            return TestResult(
                ...
                TestSeverity.MEDIUM,
                0,
                f"Rate limiting working ({success_count}/10 requests allowed)",
            )
        ...
        try:
            ...
            if campaign.total_experiments == 0:
                return TestResult(
                    ...
                    TestSeverity.HIGH,
                    0,
                    "No experiments executed during concurrent test",
                )
            ...
            if execution_time > 30:  # 30 second timeout
                return TestResult(
                    ...
                    TestSeverity.MEDIUM,
                    0,
                    f"Concurrent execution took too long: {execution_time:.2f}s",
                )
            ...
            return TestResult(
                ...
                TestSeverity.MEDIUM,
                0,
                f"Exception during concurrent execution test: {e}",
            )

    def _test_cache_performance(self) -> TestResult:
        """Test cache performance."""
        try:
            from materials_orchestrator.performance_optimizer import (
                AdaptivePerformanceCache,
            )
            ...
            if retrieved != test_value:
                return TestResult(
                    ...
                    TestSeverity.MEDIUM,
                    0,
                    "Cache get/put operation failed",
                )
            ...
            if stats["hit_rate"] == 0:
                return TestResult(
                    ...
                    TestSeverity.LOW,
                    0,
                    "Cache hit rate calculation incorrect",
                )
            ...
            return TestResult(
                ...
                TestSeverity.MEDIUM,
                0,
                f"Cache performance test passed (hit rate: {stats['hit_rate']:.1%})",
            )
            ...
            return TestResult(
                ...
                TestSeverity.MEDIUM,
                0,
                f"Exception during cache performance test: {e}",
            )
        ...
        try:
            ...
            if memory_growth > 10000:  # Arbitrary threshold
                return TestResult(
                    ...
                    TestSeverity.MEDIUM,
                    0,
                    f"Excessive memory growth: {memory_growth} objects",
                )
            ...
            return TestResult(
                ...
                TestSeverity.MEDIUM,
                0,
                f"Memory usage acceptable (growth: {memory_growth} objects)",
            )
            ...
            return TestResult(
                ...
                TestSeverity.MEDIUM,
                0,
                f"Exception during memory usage test: {e}",
            )
    ...
    def _test_response_times(self) -> TestResult:
        ...
        try:
            ...
            max_response_time = self.quality_standards["performance_thresholds"][
                "max_response_time"
            ]
            ...
            return TestResult(
                ...
                TestSeverity.MEDIUM,
                0,
                f"Exception during response time test: {e}",
            )
        ...
        try:
            ...
            if len(suggestions) != 2:
                return TestResult(
                    ...
                    TestSeverity.HIGH,
                    0,
                    f"Planner returned {len(suggestions)} suggestions instead of 2",
                )
            ...
            return TestResult(
                ...
                TestSeverity.HIGH,
                0,
                f"Exception during planner integration test: {e}",
            )
    ...
    def _test_database_integration(self) -> TestResult:
        ...
        try:
            ...
            return TestResult(
                ...
                TestSeverity.HIGH,
                0,
                "Database integration test passed (simplified)",
            )
            ...
            return TestResult(
                ...
                TestSeverity.HIGH,
                0,
                f"Exception during database integration test: {e}",
            )

    def _test_monitoring_integration(self) -> TestResult:
        """Test monitoring system integration."""
        try:
            from materials_orchestrator.health_monitoring import (
            ...
            return TestResult(
                ...
                TestSeverity.MEDIUM,
                0,
                f"Monitoring integration successful ({len(health_results)} checks)",
            )
            ...
            return TestResult(
                ...
                TestSeverity.MEDIUM,
                0,
                f"Exception during monitoring integration test: {e}",
            )

    def _test_error_recovery(self) -> TestResult:
        """Test error recovery mechanisms."""
        try:
            from materials_orchestrator.error_recovery import (
                get_global_resilient_executor,
            )
            ...
            result, success = executor.execute_with_recovery(
                failing_function, operation_name="test_error_recovery"
            )
            ...
            if success:
                return TestResult(
                    ...
                    TestSeverity.MEDIUM,
                    0,
                    "Error recovery incorrectly reported success for failing function",
                )
            ...
            return TestResult(
                ...
                TestSeverity.MEDIUM,
                0,
                "Error recovery handled failure correctly",
            )
            ...
            return TestResult(
                ...
                TestSeverity.MEDIUM,
                0,
                f"Exception during error recovery test: {e}",
            )
