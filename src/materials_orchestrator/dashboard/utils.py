...
def create_property_evolution_plot(
    ...
    completed_experiments = [
        ...
        if exp.get("status") == "completed" and 
           exp.get("results", {}).get(property_name) is not None
    ]
    ...
    if target_range:
        fig.add_hrect(
            y0=target_range[0], y1=target_range[1],
...
def create_parameter_space_plot(
    ...
    for exp in experiments:
        if (exp.get("status") == "completed" and 
            exp.get("parameters", {}).get(x_param) is not None and
            exp.get("parameters", {}).get(y_param) is not None and
            exp.get("results", {}).get(color_property) is not None):
    ...
    if size_property:
        size_values = [
            exp["results"].get(size_property, 10) 
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate=(
            ...
            + (f"<br>{size_property}: %{{marker.size:.1f}}" if size_property else "")
            + "<extra></extra>"
        )
    ))
