...
def render_campaign_overview(experiments: List[Dict[str, Any]], campaign_id: Optional[str] = None):
    ...
    failed_experiments = len([exp for exp in experiments if exp.get("status") == "failed"])
    running_experiments = len([exp for exp in experiments if exp.get("status") == "running"])
    ...
    with col2:
        st.metric("Successful", successful_experiments, delta=f"{successful_experiments/total_experiments:.1%}" if total_experiments > 0 else "0%")
    with col3:
    ...
    if total_experiments > 0:
        progress_data = {
            ...
            "Count": [
                ...
                failed_experiments, 
                running_experiments,
                max(0, total_experiments - successful_experiments - failed_experiments - running_experiments)
            ]
        }
    ...
    if completed_experiments:
        ...
        for exp in completed_experiments:
            ...
            if bandgap is not None:
                score = -abs(bandgap - target_bandgap)  # Negative for sorting (higher is better)
        ...
        for i, material in enumerate(best_materials[:5]):
            with st.expander(f"#{i+1} - Experiment {material['experiment_id']} (Band gap: {material['band_gap']:.3f} eV)"):
...
class CampaignMonitor:
    ...
    def _render_realtime_metrics(self, experiments: List[Dict[str, Any]]):
        ...
        recent_experiments = experiments[-10:] if len(experiments) >= 10 else experiments
        recent_success_rate = len([exp for exp in recent_experiments if exp.get("status") == "completed"]) / len(recent_experiments)
        ...
        with col3:
            st.metric("Experiments Today", len([exp for exp in experiments if self._is_today(exp.get("timestamp", ""))]))
    ...
    def _render_property_evolution(self, experiments: List[Dict[str, Any]]):
        ...
        if data:
            ...
            # Add target range
            fig.add_hline(y=1.2, line_dash="dash", line_color="green", annotation_text="Target Min")
            fig.add_hline(y=1.6, line_dash="dash", line_color="green", annotation_text="Target Max")
            fig.add_hline(y=1.4, line_dash="dot", line_color="red", annotation_text="Optimal")
    ...
    def _render_parameter_heatmap(self, experiments: List[Dict[str, Any]]):
        ...
        if data:
            ...
            numeric_params = [col for col in df.columns if col != "band_gap" and pd.api.types.is_numeric_dtype(df[col])]
