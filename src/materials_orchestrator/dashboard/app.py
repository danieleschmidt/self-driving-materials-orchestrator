...
from .components import render_campaign_overview, render_robot_status, render_experiment_feed
...
def create_dashboard_app():
    ...
    if available_campaigns:
        selected_campaign = st.sidebar.selectbox(
            "Select Campaign",
            ["All Campaigns"] + available_campaigns,
            index=0
        )
    else:
...
def render_dashboard_content(db: ExperimentDatabase, selected_campaign: str):
    ...
    # Load data
    if selected_campaign == "All Campaigns" or selected_campaign == "No campaigns found":
        experiments = db.query_experiments(limit=1000)
        campaign_filter = None
    else:
        experiments = db.query_experiments(
            filter_criteria={"campaign_id": selected_campaign},
            limit=1000
        )
...
def render_robot_status_tab():
    ...
    robot_data = [
        {"id": "synthesis_robot", "type": "Chemspeed", "status": "idle", "uptime": "12.5 hrs", "last_action": "heat"},
        {"id": "liquid_handler", "type": "Opentrons", "status": "busy", "uptime": "8.2 hrs", "last_action": "dispense"},
        {"id": "characterization_robot", "type": "Custom", "status": "idle", "uptime": "15.1 hrs", "last_action": "move"},
    ]
    
    instrument_data = [
        {"id": "xrd", "type": "Bruker D8", "status": "idle", "last_measurement": "5 min ago"},
        {"id": "uv_vis", "type": "Shimadzu", "status": "measuring", "last_measurement": "Running"},
        {"id": "pl_spectrometer", "type": "Horiba", "status": "idle", "last_measurement": "1 hr ago"},
    ]
    ...
    for robot in robot_data:
        ...
        with col2:
            st.write(robot['type'])
        with col3:
            status_class = "robot-status-online" if robot['status'] == "idle" else "robot-status-offline"
            st.markdown(f"<span class='{status_class}'>{robot['status']}</span>", unsafe_allow_html=True)
        with col4:
    ...
    for instrument in instrument_data:
        ...
        with col3:
            status_class = "robot-status-online" if instrument['status'] == "idle" else "robot-status-offline"
            st.markdown(f"<span class='{status_class}'>{instrument['status']}</span>", unsafe_allow_html=True)
        with col4:
...
def render_analytics_tab(experiments: List[Dict[str, Any]]):
    ...
    with col1:
        if "band_gap" in df.columns:
            fig = px.line(
                ...
                labels={"experiment_number": "Experiment #", "band_gap": "Band Gap (eV)"}
            )
    ...
    with col2:
        if "efficiency" in df.columns:
            fig = px.line(
                ...
                labels={"experiment_number": "Experiment #", "efficiency": "Efficiency (%)"}
            )
            st.plotly_chart(fig, use_container_width=True)
    ...
    numeric_params = [col for col in df.columns if col.startswith(("temperature", "precursor", "reaction", "pH"))]
...
def get_available_campaigns(db: ExperimentDatabase) -> List[str]:
    ...
    try:
        ...
        campaign_ids = list(set(
            exp.get("campaign_id") for exp in experiments 
            if exp.get("campaign_id")
        ))
