...
class AdvancedQualityAssurance:
    ...
    async def assess_campaign_quality(self,
                                    ...
                                    campaign_id: str) -> Dict[str, QualityAssessment]:
        ...
        assessments[QualityMetric.REPRODUCIBILITY] = await self._assess_reproducibility(experiments)
        ...
        assessments[QualityMetric.COMPLETENESS] = await self._assess_completeness(experiments)
        assessments[QualityMetric.CONSISTENCY] = await self._assess_consistency(experiments)
        assessments[QualityMetric.STATISTICAL_SIGNIFICANCE] = await self._assess_statistical_significance(experiments)
        assessments[QualityMetric.EXPERIMENTAL_VALIDITY] = await self._assess_experimental_validity(experiments)
        assessments[QualityMetric.DATA_INTEGRITY] = await self._assess_data_integrity(experiments)
    ...
    async def _assess_reproducibility(self, experiments: List[Dict[str, Any]]) -> QualityAssessment:
        ...
        if not reproducibility_scores:
            overall_score = 0.0
            recommendations = ["Insufficient replicate experiments for reproducibility assessment"]
        else:
            overall_score = np.mean(reproducibility_scores)
            recommendations = []
            
            if overall_score < self.min_reproducibility_score:
                recommendations.append(f"Reproducibility below threshold ({overall_score:.3f} < {self.min_reproducibility_score})")
        ...
        return QualityAssessment(
            ...
            details={
                ...
                'coefficient_of_variations': property_cvs if 'property_cvs' in locals() else []
            },
            recommendations=recommendations
        )
    
    async def _assess_accuracy(self, experiments: List[Dict[str, Any]]) -> QualityAssessment:
    ...
    async def _assess_precision(self, experiments: List[Dict[str, Any]]) -> QualityAssessment:
        ...
        for group in groups:
            ...
            for prop_name in ['band_gap', 'efficiency', 'stability']:
                ...
                if len(values) >= 3:
                    ...
                    rel_std = np.std(values) / np.mean(values) if np.mean(values) > 0 else 1.0
    ...
    async def _assess_completeness(self, experiments: List[Dict[str, Any]]) -> QualityAssessment:
        ...
        for exp in experiments:
            ...
            param_completeness = sum(1 for p in expected_parameters if p in parameters) / len(expected_parameters)
            
            # Check result completeness
            result_completeness = sum(1 for r in expected_results if r in results) / len(expected_results)
            ...
            for param in expected_parameters:
                if param not in parameters:
                    missing_data_summary[f"missing_{param}"] = missing_data_summary.get(f"missing_{param}", 0) + 1
            
            for result in expected_results:
                if result not in results:
                    missing_data_summary[f"missing_{result}"] = missing_data_summary.get(f"missing_{result}", 0) + 1
        
        overall_score = complete_experiments / total_experiments if total_experiments > 0 else 0.0
        
        recommendations = []
        if overall_score < self.min_data_completeness:
            recommendations.append(f"Data completeness below threshold ({overall_score:.3f} < {self.min_data_completeness})")
            recommendations.append("Ensure all required parameters and results are recorded")
            ...
            for key, count in missing_data_summary.items():
                if count > total_experiments * 0.1:  # More than 10% missing
                    recommendations.append(f"Frequently missing: {key.replace('missing_', '')} ({count}/{total_experiments} experiments)")
    ...
    async def _assess_consistency(self, experiments: List[Dict[str, Any]]) -> QualityAssessment:
        ...
        return QualityAssessment(
            ...
            level=self._score_to_quality_level(overall_score),
            details={
                'parameter_consistency_scores': consistency_scores[:-1] if consistency_scores else [],
                'physics_consistency_score': physics_consistency,
                'parameter_ranges': {k: {'min': min(v), 'max': max(v), 'count': len(v)} 
                                   for k, v in parameter_ranges.items()}
            },
            recommendations=recommendations
        )
    
    async def _assess_statistical_significance(self, experiments: List[Dict[str, Any]]) -> QualityAssessment:
            ...
            for prop_name in ['band_gap', 'efficiency', 'stability']:
                ...
                if len(values) >= 3:
                    ...
                    if len(values) >= 5 and std_val > 0:
                        ...
                        t_stat = abs(mean_val - 1.5) / (std_val / (len(values) ** 0.5))  # Assuming baseline of 1.5
                        ...
                        significance_tests.append({
                            ...
                            'property': prop_name,
                            'p_value': p_value,
                            'significant': p_value < self.min_statistical_significance,
                            'sample_size': len(values)
                        })
        
        # Calculate overall significance score
        significant_tests = [t for t in significance_tests if t['significant']]
        overall_score = len(significant_tests) / len(significance_tests) if significance_tests else 0.0
    ...
    async def _assess_experimental_validity(self, experiments: List[Dict[str, Any]]) -> QualityAssessment:
        ...
        for issue_type, count in issue_counts.items():
            if count > len(experiments) * 0.1:
                recommendations.append(f"Frequent {issue_type} issues: {count} occurrences")
    ...
    async def _assess_data_integrity(self, experiments: List[Dict[str, Any]]) -> QualityAssessment:
        ...
        if len(anomalies) > len(experiments) * 0.05:  # More than 5% anomalies
            recommendations.append(f"High anomaly rate: {len(anomalies)} in {len(experiments)} experiments")
    ...
    def _group_similar_experiments(self, 
                                 experiments: List[Dict[str, Any]], 
                                 tolerance: float = 0.1) -> List[List[Dict[str, Any]]]:
    ...
    def _are_experiments_similar(self, 
                               exp1: Dict[str, Any], 
                               exp2: Dict[str, Any], 
    ...
    def _group_by_conditions(self, experiments: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def _is_outlier_experiment(self, 
                             experiment: Dict[str, Any], 
                             all_experiments: List[Dict[str, Any]]) -> bool:
    ...
    def _update_quality_history(self, 
                              campaign_id: str, 
                              assessments: Dict[QualityMetric, QualityAssessment]):
    ...
    def generate_quality_report(self, 
                              assessments: Dict[QualityMetric, QualityAssessment]) -> Dict[str, Any]:
