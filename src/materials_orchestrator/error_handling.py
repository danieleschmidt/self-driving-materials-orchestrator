...
class RecoveryStrategy:
    ...
    def attempt_recovery(self, error: MaterialsError, original_function: Callable, *args, **kwargs) -> Any:
...
class RetryRecoveryStrategy(RecoveryStrategy):
    ...
    def __init__(self, name: str = "retry", max_retries: int = 3, backoff_seconds: float = 1.0):
    ...
    def can_handle(self, error: MaterialsError) -> bool:
        ...
        return error.category in transient_categories and error.retry_count < self.max_retries
    
    def attempt_recovery(self, error: MaterialsError, original_function: Callable, *args, **kwargs) -> Any:
        """Attempt recovery by retrying with backoff."""
        for attempt in range(self.max_retries):
            if attempt > 0:
                delay = self.backoff_seconds * (2 ** (attempt - 1))  # Exponential backoff
                logger.info(f"Retrying {original_function.__name__} in {delay}s (attempt {attempt + 1}/{self.max_retries})")
                time.sleep(delay)
            ...
            try:
                ...
                logger.info(f"Recovery successful for {error.error_id} after {attempt + 1} attempts")
                return result
...
class ParameterAdjustmentStrategy(RecoveryStrategy):
    ...
    def __init__(self, name: str = "parameter_adjustment", adjustment_rules: Optional[Dict[str, Any]] = None):
    ...
    def attempt_recovery(self, error: MaterialsError, original_function: Callable, *args, **kwargs) -> Any:
        ...
        logger.info(f"Attempting recovery with adjusted parameters for {error.error_id}")
    ...
    def _adjust_parameters(self, parameters: Dict[str, Any], error: MaterialsError) -> Dict[str, Any]:
...
class ErrorHandler:
    ...
    def __init__(self):
        ...
        self._category_patterns: Dict[ErrorCategory, List[str]] = {
            ErrorCategory.VALIDATION: ["invalid", "out of bounds", "validation", "parameter"],
            ...
            ErrorCategory.SECURITY: ["security", "permission", "access", "authentication"],
        }
    ...
    def handle_error(
        ...
        **kwargs
        ...
        self._error_counts[error.category.value] = self._error_counts.get(error.category.value, 0) + 1
    ...
    def _assess_severity(self, exception: Exception) -> ErrorSeverity:
        ...
        # Critical errors
        critical_indicators = [
            "security", "authentication", "permission", "memory",
            "system", "critical", "fatal", "corruption"
        ]
        
        if any(indicator in error_msg or indicator in exception_type for indicator in critical_indicators):
            return ErrorSeverity.CRITICAL
        
        # High severity errors
        high_indicators = [
            "database", "connection", "network", "robot", "hardware"
        ]
        
        if any(indicator in error_msg or indicator in exception_type for indicator in high_indicators):
            return ErrorSeverity.HIGH
        
        # Medium severity errors
        medium_indicators = [
            "validation", "parameter", "computation", "timeout"
        ]
        
        if any(indicator in error_msg or indicator in exception_type for indicator in medium_indicators):
            return ErrorSeverity.MEDIUM
        
        return ErrorSeverity.LOW
    
    def _attempt_recovery(self, error: MaterialsError, original_function: Callable, *args, **kwargs) -> Optional[Any]:
        ...
        for strategy in self._recovery_strategies:
            if strategy.can_handle(error):
                ...
                try:
                    result = strategy.attempt_recovery(error, original_function, *args, **kwargs)
                    error.recovery_successful = True
