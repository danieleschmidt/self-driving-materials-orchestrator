...
class BayesianPlanner(BasePlanner):
    ...
    def _prepare_data(self, previous_results: List[Dict[str, Any]], param_space: Dict[str, tuple]):
    ...
    def suggest_next(
        ...
        if not NUMPY_AVAILABLE or not SKLEARN_AVAILABLE:
            ...
            return RandomPlanner().suggest_next(n_suggestions, param_space, previous_results)
        ...
        for _ in range(n_suggestions):
            ...
            candidates = []
            for _ in range(1000):
                candidate = [np.random.uniform(low, high) for low, high in param_space.values()]
            ...
            # Convert to dict
            suggestion = {name: float(val) for name, val in zip(self._param_names, best_candidate)}
