...
from materials_orchestrator import (
    ...
    create_database, create_health_monitor
)
...
def run_campaign(
    ...
    param_space_file: str = typer.Option(None, "--params", "-p", help="JSON file with parameter space"),
    max_experiments: int = typer.Option(100, "--max-exp", "-m", help="Maximum experiments"),
    initial_samples: int = typer.Option(20, "--init", "-i", help="Initial random samples"),
    planner: str = typer.Option("bayesian", "--planner", help="Optimization planner"),
    material_system: str = typer.Option("general", "--system", "-s", help="Material system"),
    ...
    # Run campaign
    with typer.progressbar(length=max_experiments, label="Running experiments") as progress:
    ...
    typer.echo(f"Duration: {campaign.duration:.2f} hours" if campaign.duration else "Duration: < 0.01 hours")
...
def analyze_results(
    ...
    cost_savings = (1 - data['results']['total_experiments'] / traditional_estimate) * 100
...
def _show_convergence_plot(convergence: List[Dict]) -> None:
    ...
    # Simple text plot
    width = 50
    for i, norm_val in enumerate(normalized[::max(1, len(normalized)//20)]):  # Sample points
        bar_length = int(norm_val * width)
        bar = "█" * bar_length + "░" * (width - bar_length)
        typer.echo(f"   {i*max(1, len(normalized)//20):3d}: {bar} {fitness_values[i*max(1, len(normalized)//20)]:.3f}")
...
def create_param_space(
    ...
    output_file: str = typer.Option("param_space.json", "--output", "-o", help="Output file"),
) -> None:
...
def list_examples() -> None:
    ...
    for example in examples:
        ...
        typer.echo(f"   Command: materials-orchestrator run-campaign {example['target_property']} 0.0 2.0 --system {example['material_system']}")
