    ...
    def __post_init__(self):
        ...
        if not self.lawful_bases:
            if self.regulation == ComplianceRegulation.GDPR:
                self.lawful_bases = [ConsentType.EXPLICIT, ConsentType.LEGITIMATE_INTEREST]
...
class GlobalComplianceManager:
    ...
    def _initialize_processing_purposes(self):
        ...
        self.processing_purposes["materials_research"] = DataProcessingPurpose(
            ...
            data_categories=["experimental_data", "synthesis_parameters", "property_measurements"],
    ...
    def register_data_processing(
        ...
        for regulation in applicable_regulations:
            ...
            if policy.consent_management_required and not consent_obtained:
                if purpose.legal_basis == ConsentType.EXPLICIT:
                    raise ValueError(f"Explicit consent required for {regulation.value}")
        ...
        self._audit_log(
            ...
            details={
                ...
                "legal_basis": legal_basis.value if legal_basis else purpose.legal_basis.value,
    ...
    def _get_applicable_regulations(self, data_subject_id: str) -> List[ComplianceRegulation]:
    ...
    def _pseudonymize_identifier(self, identifier: str) -> str:
        ...
        return hashlib.sha256((identifier + self.pseudonymization_salt).encode()).hexdigest()[:16]
    ...
    def withdraw_consent(self, data_subject_id: str, purpose_id: str):
        ...
        for consent_id, consent in self.consent_records.items():
            if (consent["data_subject_id"] == pseudonymized_id and
                consent["purpose_id"] == purpose_id and
                ...
                if consent["purpose_id"] == purpose_id:
                    ...
                    if current_time - recorded_at > retention_limit:
                        violations.append({
                            ...
                            "days_overdue": (current_time - recorded_at - retention_limit).days
                        })
        
        return violations
    
    def generate_compliance_report(self, regulation: ComplianceRegulation) -> Dict[str, Any]:
    ...
    def export_audit_logs(self, format: str = "json") -> str:
        ...
        if format == "json":
            ...
            for log in self.audit_logs:
                ...
                lines.append(f"{log['timestamp']},{log['event_type']},\"{details_str}\"")
            return "\n".join(lines)
        else:
