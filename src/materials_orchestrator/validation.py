...
class ExperimentValidator:
    ...
    def validate_experiment_data(
        self, parameters: Dict[str, Any], results: Optional[Dict[str, Any]] = None
    ...
    def validate_parameters(self, parameters: Dict[str, Any]) -> List[ValidationResult]:
        ...
        for param_name, value in parameters.items():
            if param_name in rules:
                param_results = self._validate_single_parameter(
                    param_name, value, rules[param_name]
                )
    ...
    def validate_results(self, results: Dict[str, Any]) -> List[ValidationResult]:
        ...
        for result_name, value in results.items():
            if result_name in rules:
                result_validations = self._validate_single_parameter(
                    result_name, value, rules[result_name]
                )
                validation_results.extend(result_validations)
            else:
    ...
    def _validate_single_parameter(
        self, name: str, value: Any, rule: Dict[str, Any]
    ) -> List[ValidationResult]:
    ...
    def cross_validate(
        self, parameters: Dict[str, Any], results: Dict[str, Any]
    ) -> List[ValidationResult]:
    ...
    def calculate_quality_metrics(
        ...
        required_params = [
            name
            for name, rule in self.validation_rules["experiment_parameters"].items()
            if rule.get("required", False)
        ]
        present_required = sum(1 for param in required_params if param in parameters)
        completeness = (
            present_required / len(required_params) if required_params else 1.0
        )
        ...
        outlier_results = [
            r for r in validation_results if "outlier" in r.message.lower()
        ]
        outlier_score = (
            len(outlier_results) / len(validation_results)
    ...
    def generate_quality_report(
        ...
        valid_count = sum(
            1 for r in validation_results if r.status == ValidationStatus.VALID
        )
        warning_count = sum(
            1 for r in validation_results if r.status == ValidationStatus.WARNING
        )
        invalid_count = sum(
            1 for r in validation_results if r.status == ValidationStatus.INVALID
        )
        error_count = sum(
            1 for r in validation_results if r.status == ValidationStatus.ERROR
        )
