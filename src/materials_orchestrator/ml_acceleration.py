...
class PropertyPredictor:
    ...
    def _extract_features(
        ...
        feature_map["total_conc"] = (
            feature_map["precursor_A_conc"] + feature_map["precursor_B_conc"]
        )
        feature_map["conc_ratio"] = feature_map["precursor_A_conc"] / max(
            feature_map["precursor_B_conc"], 0.001
        )
        feature_map["temp_time_product"] = (
            feature_map["temperature"] * feature_map["reaction_time"]
        )
            ...
            try:
                ...
                if len(X) > 10:
                    scores = cross_val_score(
                        model, X_scaled, y, cv=min(5, len(X) // 2), scoring="r2"
                    )
...
class IntelligentOptimizer:
        ...
        for suggestion in self.recent_suggestions[-10:]:  # Check last 10 suggestions
            if self._parameters_match(suggestion.parameters, parameters, tolerance=0.1):
                ...
                improvement_score = min(
                    suggestion.expected_improvement / max(abs(target_value - 1.4), 0.1),
                    2.0,
                )
    ...
    def _parameters_match(
        ...
        for key in params1:
            if key in params2:
                if (
                    abs(params1[key] - params2[key]) / max(abs(params1[key]), 0.1)
                    > tolerance
                ):
                    return False
        return True
    ...
    def _select_strategies(
        self, strategy_scores: Dict[str, float], n_needed: int
    ...
    def _expected_improvement(
        ...
        for result in previous_results:
            ...
            if value is not None:
                if best_value is None or abs(value - 1.4) < abs(
                    best_value - 1.4
                ):  # Closer to target
                    best_value = value
        ...
        for candidate in candidates:
            ...
            if prediction:
                ...
                improvement = max(
                    0, abs(prediction.predicted_value - 1.4) - abs(best_value - 1.4)
                )
    ...
    def _generate_candidates(
        self, param_space: Dict[str, Tuple[float, float]], n_candidates: int
    ) -> List[Dict[str, float]]:
    ...
    def _calculate_diversity(
        self, candidate: Dict[str, float], existing_params: List[Dict[str, float]]
    ) -> float:
