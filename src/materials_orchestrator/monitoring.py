...
class HealthMonitor:
    ...
    def _process_alerts(self, health_results: List[HealthCheck], metrics: SystemMetrics) -> None:
        ...
        for result in health_results:
            if result.status in [HealthStatus.WARNING, HealthStatus.CRITICAL]:
                ...
                recent_alerts = [
                    ...
                    if a.get("check_name") == result.name and a.get("status") == result.status.value
                ]
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {
                "cpu_percent": self._last_metrics.cpu_percent if self._last_metrics else 0,
                "memory_percent": self._last_metrics.memory_percent if self._last_metrics else 0,
                "disk_percent": self._last_metrics.disk_percent if self._last_metrics else 0,
                "experiment_throughput": self._last_metrics.experiment_throughput if self._last_metrics else 0,
            },
            "active_alerts": len([a for a in self.alerts[-10:] if a.get("status") in ["warning", "critical"]]),
