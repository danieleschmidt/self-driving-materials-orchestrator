...
class ResilientExecutor:
    ...
    def execute_with_recovery(self, 
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                # Try recovery
                recovery_result = self._attempt_recovery(error_record, operation, args, kwargs)
                ...
                if attempt < self.max_retries:
                    if self._should_retry(e, attempt):
                        ...
                        logger.info(f"Retrying {operation_name} in {delay:.2f}s (attempt {attempt + 2}/{self.max_retries + 1})")
                        time.sleep(delay)
                        continue
                    else:
                        logger.info(f"Not retrying {operation_name} due to error type: {error_type}")
                        break
                else:
        ...
        if fallback_result[1]:
            ...
            if last_exception:
                ...
                raise RuntimeError(f"Operation {operation_name} failed without specific exception")
    
    def _record_error(self, error_type: str, message: str, severity: ErrorSeverity, context: Dict[str, Any]) -> ErrorRecord:
    ...
    def _attempt_recovery(self, error_record: ErrorRecord, operation: Callable, args: tuple, kwargs: dict) -> Tuple[Any, bool]:
        ...
        if error_type in self.recovery_handlers:
            try:
                ...
                result = self.recovery_handlers[error_type](error_record, operation, args, kwargs)
    ...
    def _recover_connection_error(self, error_record: ErrorRecord, operation: Callable, args: tuple, kwargs: dict) -> Tuple[Any, bool]:
        ...
        try:
            ...
            if hasattr(operation, '__self__') and hasattr(operation.__self__, 'reconnect'):
    ...
    def _recover_timeout_error(self, error_record: ErrorRecord, operation: Callable, args: tuple, kwargs: dict) -> Tuple[Any, bool]:
    ...
    def _recover_validation_error(self, error_record: ErrorRecord, operation: Callable, args: tuple, kwargs: dict) -> Tuple[Any, bool]:
        ...
        if 'parameters' in error_record.context:
            try:
                ...
                sanitized_params = InputValidator.validate_experiment_parameters(error_record.context['parameters'])
    ...
    def _should_retry(self, error: Exception, attempt: int) -> bool:
        ...
        no_retry_errors = ['KeyboardInterrupt', 'SystemExit', 'MemoryError', 'ImportError']
    ...
    def _is_circuit_open(self, operation: str) -> bool:
        ...
        if breaker.state == "CLOSED":
            ...
            if (breaker.last_failure_time and 
                datetime.now() - breaker.last_failure_time > timedelta(seconds=self.circuit_breaker_config['recovery_timeout'])):
    ...
    def _record_success(self, operation: str):
        ...
        if operation in self.circuit_breakers:
            ...
            if breaker.state == "HALF_OPEN":
                ...
                if breaker.success_count_in_half_open >= self.circuit_breaker_config['success_threshold']:
    ...
    def _record_failure(self, operation: str):
        ...
        if breaker.failure_count >= self.circuit_breaker_config['failure_threshold']:
            breaker.state = "OPEN"
            logger.warning(f"Circuit breaker for {operation} OPENED due to {breaker.failure_count} failures")
    ...
    def get_error_statistics(self) -> Dict[str, Any]:
        ...
        with self._error_count_lock:
            ...
            now = datetime.now()
            recent_errors = [e for e in self.error_history if (now - e.timestamp).total_seconds() < 3600]  # Last hour
            ...
            return {
                ...
                "recovery_rate": recovery_stats["successful"] / max(recovery_stats["attempted"], 1),
                "circuit_breakers": {name: state.state for name, state in self.circuit_breakers.items()}
            }
