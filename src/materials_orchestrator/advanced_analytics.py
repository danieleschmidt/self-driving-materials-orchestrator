...
class AdvancedAnalyzer:
    ...
    def analyze_campaign(self, campaign_results) -> CampaignAnalytics:
        ...
        analytics.acceleration_factor = self._calculate_acceleration_factor(campaign_results)
        ...
        # Statistical analysis
        analytics.parameter_importance = self._analyze_parameter_importance(campaign_results)
        analytics.correlation_matrix = self._calculate_correlation_matrix(campaign_results)
        analytics.property_statistics = self._calculate_property_statistics(campaign_results)
        
        # Optimization insights
        analytics.convergence_analysis = self._analyze_convergence(campaign_results)
        analytics.exploration_coverage = self._calculate_exploration_coverage(campaign_results)
        analytics.exploitation_efficiency = self._calculate_exploitation_efficiency(campaign_results)
        
        # Quality metrics
        analytics.reproducibility_score = self._calculate_reproducibility(campaign_results)
        analytics.confidence_intervals = self._calculate_confidence_intervals(campaign_results)
    ...
    def _calculate_success_rate(self, campaign_results) -> float:
        ...
        if campaign_results.total_experiments == 0:
            return 0.0
        return campaign_results.successful_experiments / campaign_results.total_experiments
    ...
    def _analyze_parameter_importance(self, campaign_results) -> Dict[str, float]:
        ...
        for param in param_names:
            try:
                ...
                property_values = [exp.results.get(target_property, 0) for exp in successful_exp]
                
                if len(set(param_values)) > 1:  # Parameter has variation
                    # Calculate correlation coefficient
                    correlation = self._calculate_correlation(param_values, property_values)
    ...
    def _calculate_correlation_matrix(self, campaign_results) -> Dict[str, Dict[str, float]]:
        """Calculate correlation matrix between all parameters and properties."""
        experiments = [exp for exp in campaign_results.experiments if exp.status == "completed"]
        ...
        for var1 in all_vars:
            ...
            for var2 in all_vars:
                ...
                correlation_matrix[var1][var2] = self._calculate_correlation(values1, values2)
        
        return correlation_matrix
    
    def _calculate_property_statistics(self, campaign_results) -> Dict[str, Dict[str, float]]:
    ...
    def _analyze_convergence(self, campaign_results) -> Dict[str, Any]:
        ...
        analysis = {
            'total_improvements': len([i for i in range(1, len(fitness_values)) 
        ...
        if len(fitness_values) > 1:
            analysis['improvement_rate'] = (fitness_values[-1] - fitness_values[0]) / len(fitness_values)
        ...
        if len(fitness_values) >= 5:
            ...
            analysis['plateau_analysis'] = {
                ...
                'plateau_variance': statistics.variance(last_quarter) if len(last_quarter) > 1 else 0,
                'is_converged': statistics.variance(last_quarter) < 0.001 if len(last_quarter) > 1 else False
            }
        ...
        for group in parameter_groups:
            if len(group) >= 2:
                ...
                if property_values:
                    ...
                    if mean_val != 0:
                        std_val = statistics.stdev(property_values) if len(property_values) > 1 else 0
                        cv = std_val / abs(mean_val)
                        reproducibility_scores.append(1.0 / (1.0 + cv))  # Higher score for lower variation
        
        return statistics.mean(reproducibility_scores) if reproducibility_scores else 0.5
    
    def _group_similar_experiments(self, experiments: List, tolerance: float = 0.1) -> List[List]:
        ...
        for i, exp1 in enumerate(experiments):
            ...
            for j, exp2 in enumerate(experiments[i+1:], i+1):
                ...
                if self._parameters_similar(exp1.parameters, exp2.parameters, tolerance):
                    group.append(exp2)
    ...
    def _parameters_similar(self, params1: Dict, params2: Dict, tolerance: float) -> bool:
    ...
    def _calculate_confidence_intervals(self, campaign_results) -> Dict[str, Tuple[float, float]]:
    ...
    def _detect_outliers(self, campaign_results) -> List[str]:
        ...
        target_property = campaign_results.objective.target_property
        values = [exp.results.get(target_property, 0) for exp in experiments 
    ...
    def generate_insights_report(self, analytics: CampaignAnalytics) -> Dict[str, Any]:
        ...
        insights = {
            ...
            'quality_assessment': {
                'reproducibility_rating': self._rate_reproducibility(analytics.reproducibility_score),
                'data_quality_issues': [],
        ...
        if analytics.success_rate > 0.9:
            insights['performance_summary']['key_strengths'].append("Excellent experiment success rate")
        elif analytics.success_rate < 0.7:
            insights['performance_summary']['improvement_areas'].append("Low experiment success rate")
            insights['performance_summary']['recommendations'].append("Review experimental protocols")
        
        if analytics.acceleration_factor > 3:
            insights['performance_summary']['key_strengths'].append(f"Strong acceleration ({analytics.acceleration_factor:.1f}x)")
        
        # Parameter insights
        if analytics.parameter_importance:
            sorted_params = sorted(analytics.parameter_importance.items(), 
                                 key=lambda x: x[1], reverse=True)
            insights['parameter_insights']['most_important_parameters'] = sorted_params[:3]
            
            # Suggest focusing on most important parameters
            if sorted_params[0][1] > 0.4:
                insights['parameter_insights']['parameter_optimization_suggestions'].append(
        ...
        if analytics.convergence_analysis.get('is_converged'):
            insights['optimization_insights']['convergence_assessment'] = "Algorithm converged successfully"
        else:
            insights['optimization_insights']['convergence_assessment'] = "Algorithm may benefit from more iterations"
            insights['optimization_insights']['next_campaign_suggestions'].append("Continue optimization with more experiments")
