...
class LabDashboard:
    ...
    def __init__(self, lab: Optional[AutonomousLab] = None, tracker: Optional[ExperimentTracker] = None):
        self.lab = lab
    ...
    def render_main_dashboard(self):
        ...
        # Main content area
        tab1, tab2, tab3, tab4 = st.tabs(["📊 Dashboard", "🧪 Experiments", "🤖 Robots", "📈 Analytics"])
            ...
            with col1:
                ...
                st.json({
                    "parameters": {
                        ...
                        "temperature": float(exp_data["Temperature (°C)"]) if pd.notna(exp_data["Temperature (°C)"]) else None,
                        "reaction_time": float(exp_data["Time (h)"]) if pd.notna(exp_data["Time (h)"]) else None,
                        "pH": 7.2,
                        "solvent_ratio": 0.6
                    },
                    "results": {
                        "band_gap": float(exp_data["Band Gap (eV)"]) if pd.notna(exp_data["Band Gap (eV)"]) else None,
                        "efficiency": float(exp_data["Efficiency (%)"]) if pd.notna(exp_data["Efficiency (%)"]) else None,
                        "stability": 0.89 if exp_data["Status"] == "✅ Completed" else None
                    }
                })
    ...
    def _render_robots_tab(self):
            ...
            with st.container():
                ...
                with col3:
                    ...
                    if robot['status'] == "🔴 Offline":
                        if st.button(f"🔄 Restart {robot['id']}", key=f"restart_{robot['id']}"):
        ...
        hours = list(range(24))
        lh_util = [50 + 30 * abs(h - 12) / 12 + np.random.randint(-10, 10) for h in hours]
        syn_util = [60 + 20 * abs(h - 14) / 14 + np.random.randint(-5, 15) for h in hours]
        ana_util = [30 + 40 * abs(h - 10) / 10 + np.random.randint(-15, 5) for h in hours]
        
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=hours, y=lh_util, name='Liquid Handler', mode='lines+markers'))
        fig.add_trace(go.Scatter(x=hours, y=syn_util, name='Synthesizer', mode='lines+markers'))
        fig.add_trace(go.Scatter(x=hours, y=ana_util, name='Analyzer', mode='lines+markers'))
    ...
    def _render_analytics_tab(self):
        ...
        band_gap = 1.5 + (temp - 200) / 1000 + (time - 12) / 100 + (conc - 1) / 10 + np.random.normal(0, 0.1, n_points)
        ...
        cost_df["Savings ($/day)"] = cost_df["Traditional ($/day)"] - cost_df["Autonomous ($/day)"]
        
        st.dataframe(cost_df, use_container_width=True)
        
        st.success(f"💰 Daily savings: ${cost_df.loc[cost_df['Category'] == 'Total', 'Savings ($/day)'].iloc[0]}")
        st.info(f"📅 Monthly savings: ${cost_df.loc[cost_df['Category'] == 'Total', 'Savings ($/day)'].iloc[0] * 30:,}")
