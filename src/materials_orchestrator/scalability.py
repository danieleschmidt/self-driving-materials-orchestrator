...
class AutoScaler:
    ...
    def should_scale(
        ...
        if (datetime.now() - self._last_scale_action).total_seconds() < self.cooldown_seconds:
            return None
        ...
        if average_load > self.scale_up_threshold and current_workers < self.max_workers:
            scale_up_reasons.append(f"load ({average_load:.2f}) > threshold ({self.scale_up_threshold})")
        
        if queue_length > current_workers * 2 and current_workers < self.max_workers:
            scale_up_reasons.append(f"queue ({queue_length}) > 2x workers ({current_workers})")
        
        if average_response_time > 60.0 and current_workers < self.max_workers:
            scale_up_reasons.append(f"response time ({average_response_time:.1f}s) > 60s")
        ...
        if (average_response_time < 5.0 and 
            queue_length == 0 and 
            current_workers > self.min_workers):
            scale_down_reasons.append(f"response time ({average_response_time:.1f}s) < 5s")
...
class DistributedJobManager:
        ...
        with self._lock:
            if node_id in self._workers:
                ...
                jobs_to_reassign = [
                    ...
                    if job.assigned_node == node_id and job.status in ["assigned", "running"]
                ]
                
                for job in jobs_to_reassign:
                    logger.warning(f"Reassigning job {job.job_id} from failed worker {node_id}")
    ...
    def _dispatcher_loop(self):
        ...
        while self._running:
            try:
                ...
                threading.Thread(
                    target=self._execute_job_on_worker,
                    args=(job, worker),
                    daemon=True
                ).start()
        ...
        try:
            ...
            with self._lock:
                ...
                if job.retry_count < job.max_retries:
                    ...
                    logger.error(f"Job {job.job_id} failed after {job.retry_count} retries: {e}")
    ...
    def _simulate_job_execution(self, job: DistributedJob) -> Any:
        ...
        return {"result": f"Processed {job.job_type}", "timestamp": datetime.now().isoformat()}
                    ...
                    for worker in self._workers.values():
                        ...
                        if time_since_heartbeat > 120:  # 2 minutes timeout
                            if worker.status == "active":
                                ...
                                logger.warning(f"Worker {worker.node_id} marked as inactive (heartbeat timeout)")
    ...
    def _check_auto_scaling(self):
        ...
        with self._lock:
            active_workers = sum(1 for w in self._workers.values() if w.status == "active")
            queue_length = self._job_queue.qsize()
            
            if active_workers > 0:
                total_load = sum(w.load_factor for w in self._workers.values() if w.status == "active")
            ...
            average_response_time = sum(j.duration for j in recent_jobs) / len(recent_jobs) if recent_jobs else 0.0
        ...
        with self._lock:
            ...
            return {
                "workers": {
                    ...
                    "average_load": sum(w.load_factor for w in active_workers) / len(active_workers) if active_workers else 0,
                },
                "jobs": {
                ...
                "auto_scaling": {
                    ...
                    "recent_actions": self.auto_scaler.get_scaling_history()[-5:] if self.enable_auto_scaling else [],
                },
                "timestamp": datetime.now().isoformat(),
            }
...
class ExperimentBatchProcessor:
    ...
    def process_experiment_batch(
        ...
        monitor_thread = threading.Thread(
            ...
            args=(batch_id, progress_callback),
            daemon=True
        )
    ...
    def _monitor_batch(self, batch_id: str, progress_callback: Optional[Callable]):
        ...
        while True:
            with self._lock:
                ...
                batch_info["completed_jobs"] = len(batch_info["results"]) + len(batch_info["errors"])
                ...
                if batch_info["completed_jobs"] >= batch_info["total_jobs"]:
                    ...
                    logger.info(f"Batch {batch_id} completed: {batch_info['completed_jobs']} jobs")
                    
                    if progress_callback:
                        progress_callback(batch_id, batch_info["completed_jobs"], batch_info["total_jobs"])
