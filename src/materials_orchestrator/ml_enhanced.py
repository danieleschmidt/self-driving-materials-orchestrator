...
class EnhancedMLOptimizer:
    ...
    def __init__(self, target_property: str, model_type: str = "gaussian_process"):
        ...
        if not self.has_sklearn:
            logger.warning("scikit-learn not available, using advanced fallback algorithms")
    ...
    def _train_sklearn_model(self) -> ModelMetrics:
        ...
        try:
            ...
            self.metrics.accuracy = max(0, 1 - mean_absolute_error(y, y_pred) / (max(y) - min(y) + 1e-10))
            ...
            logger.info(f"Model trained - R²: {self.metrics.r2_score:.3f}, MAE: {self.metrics.mae:.3f}")
    ...
    def _train_fallback_model(self) -> ModelMetrics:
        ...
        predictions = [self._predict_fallback(exp['parameters']) for exp in self.training_data]
        actual_values = [exp['results'][self.target_property] for exp in self.training_data 
                        if self.target_property in exp['results']]
        
        if len(predictions) == len(actual_values) and len(actual_values) > 0:
            mae = sum(abs(p - a) for p, a in zip(predictions, actual_values)) / len(actual_values)
            self.metrics.mae = mae
            self.metrics.accuracy = max(0, 1 - mae / (max(actual_values) - min(actual_values) + 1e-10))
        
        self.model_trained = True
        logger.info(f"Fallback model trained with {len(self.training_data)} data points")
    ...
    def _calculate_parameter_correlations(self) -> Dict[str, float]:
        ...
        for param in all_params:
            ...
            for exp in self.training_data:
                if param in exp['parameters'] and self.target_property in exp['results']:
    ...
    def _predict_sklearn(self, parameters: Dict[str, Any]) -> Tuple[float, float]:
        ...
        try:
            ...
            param_names = sorted(set().union(*(exp['parameters'].keys() for exp in self.training_data)))
    ...
    def suggest_next_experiments(self, param_space: Dict[str, Tuple[float, float]], 
                                n_suggestions: int = 5) -> List[Dict[str, float]]:
    ...
    def _generate_suggestion(self, param_space: Dict[str, Tuple[float, float]], 
                    ...
                    if param in correlations:
                        ...
                        if abs(corr) > 0.3:  # Strong correlation
                            ...
                            if param_avg is not None:
                                ...
                                if corr > 0:
                                    value = param_avg + (high - param_avg) * random.random() * 0.7
                                else:
                                    value = param_avg - (param_avg - low) * random.random() * 0.7
