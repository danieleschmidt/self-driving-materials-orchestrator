...
class AdvancedHealthMonitor:
    ...
    def _register_default_checks(self):
        ...
        # Core system checks
        self.register_health_check("core_imports", self._check_core_imports, ComponentType.CORE)
        self.register_health_check("memory_usage", self._check_memory_usage, ComponentType.MEMORY)
        self.register_health_check("cpu_usage", self._check_cpu_usage, ComponentType.CPU)
        self.register_health_check("disk_space", self._check_disk_space, ComponentType.STORAGE)
        
        # Application-specific checks
        self.register_health_check("database_connection", self._check_database_connection, ComponentType.DATABASE)
        self.register_health_check("experiment_queue", self._check_experiment_queue, ComponentType.CORE)
        self.register_health_check("robot_connectivity", self._check_robot_connectivity, ComponentType.ROBOTS)
    
    def register_health_check(self, name: str, check_function: Callable, component_type: ComponentType):
        ...
        for check_name, (check_function, component_type) in self.health_checks.items():
            try:
                ...
                if not isinstance(component_health, ComponentHealth):
                    ...
                    if isinstance(component_health, tuple) and len(component_health) >= 2:
                ...
                perf_metric = HealthMetric(
                    ...
                    unit="seconds",
                    status=HealthStatus.HEALTHY if check_duration < 1.0 else HealthStatus.WARNING
                )
                ...
                if len(self.metrics_history[check_name]) > 1000:
                    self.metrics_history[check_name] = self.metrics_history[check_name][-500:]
                ...
                error_component = ComponentHealth(
                    ...
                    last_error=str(e),
                    error_count=self.components.get(check_name, ComponentHealth(check_name, component_type, HealthStatus.UNKNOWN)).error_count + 1
                )
    ...
    def _trigger_alerts(self, component_health: ComponentHealth):
        ...
        alert_message = f"Health Alert: {component_health.name} is {component_health.status.value}"
        
        if component_health.metrics:
            critical_metrics = [m for m in component_health.metrics if m.status == HealthStatus.CRITICAL]
            if critical_metrics:
    ...
    def _check_core_imports(self) -> ComponentHealth:
        ...
        metrics.append(HealthMetric(
            ...
            value=success_rate,
            unit="fraction",
            status=HealthStatus.HEALTHY if success_rate == 1.0 else HealthStatus.CRITICAL
        ))
        ...
        if self.has_psutil:
            try:
                ...
                metrics.append(HealthMetric(
                    ...
                    value=available_gb,
                    unit="GB",
                    status=HealthStatus.CRITICAL if available_gb < 0.5 else HealthStatus.HEALTHY
                ))
        ...
        if self.has_psutil:
            try:
                ...
                metrics.append(HealthMetric(
                    ...
                    value=available_gb,
                    unit="GB",
                    status=HealthStatus.CRITICAL if available_gb < 1.0 else HealthStatus.HEALTHY
                ))
    ...
    def get_overall_health(self) -> Tuple[HealthStatus, Dict[str, Any]]:
        ...
        summary = {
            ...
            "last_check": max(comp.last_check for comp in self.components.values()).isoformat(),
