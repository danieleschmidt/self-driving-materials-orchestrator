...
class SecurityManager:
            ...
            try:
                ...
                for user_data in config.get("users", []):
                    user = User(
                        ...
                        permissions=set(OperationType(p) for p in user_data.get("permissions", [])),
    ...
    def create_user(
        ...
        permissions = custom_permissions or self.default_permissions.get(access_level, set())
    ...
    def require_permission(self, user: User, operation: OperationType) -> None:
        ...
        if not self.check_permission(user, operation):
            ...
            raise PermissionError(f"User {user.username} does not have permission for {operation.value}")
    ...
    def record_failed_login(self, username: str) -> None:
        ...
        if username in self.users:
            ...
            if user.failed_login_attempts >= 5:
                ...
                logger.warning(f"User {username} locked due to repeated failed login attempts")
    ...
    def get_security_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "locked_users": len([u for u in self.users.values() if self._is_user_locked(u)]),
            "api_keys_issued": len(self.api_keys),
            "events_last_24h": len(recent_events),
            "failed_events_last_24h": len(failed_events),
            "recent_failed_logins": len([e for e in failed_events if e.event_type == "failed_login"]),
            "unauthorized_attempts": len([e for e in failed_events if e.event_type == "unauthorized_access"])
        }
            ...
            if param in safe_ranges:
                try:
                    ...
                    if min_val <= float_value <= max_val:
                        ...
                        logger.warning(f"Parameter {param} value {float_value} outside safe range {safe_ranges[param]}")
                        # Clamp to safe range
