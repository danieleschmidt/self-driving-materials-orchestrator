...
class AdaptiveLearningEngine:
    ...
    def predict_experiment_success(
        ...
        for pattern_id, pattern in self.patterns.items():
            if (
                pattern.pattern_type == "parameter_correlation"
                ...
                if match_score > 0.7:
                    success_probability = max(
                        success_probability, pattern.success_rate * match_score
                    )
        ...
        if similar_experiments:
            similar_success_rate = sum(
                exp.success for exp in similar_experiments
            ) / len(similar_experiments)
            success_probability = (success_probability + similar_success_rate) / 2
            reasoning.append(
                f"Similar experiments success rate: {similar_success_rate:.2f}"
            )

        reasoning_text = (
            "; ".join(reasoning) if reasoning else "No specific patterns found"
        )
        return success_probability, reasoning_text

    def recommend_parameters(
        self, objective, param_space: Dict[str, Tuple], num_recommendations: int = 3
    ) -> List[Dict[str, float]]:
        ...
        if successful_experiments:
            ...
            for exp in best_experiments[:num_recommendations]:
                variation = self._generate_parameter_variation(
                    exp.parameters, param_space
                )
    ...
    def get_learning_insights(self) -> Dict[str, Any]:
        ...
        if len(self.patterns) < 3:
            insights["recommendations"].append(
                "Increase experiment variety to discover patterns"
            )
    ...
    def _detect_parameter_correlations(
        ...
        for param_name in outcome.parameters.keys():
            ...
            if len(param_values) >= 10:
                ...
                if (
                    ...
                    and abs(correlation)
                    > self.adaptive_thresholds["correlation_threshold"]
                ):
                    pattern = LearningPattern(
                        ...
                        success_rate=sum(
                            exp.success for exp in self.experiment_history[-50:]
                        )
                        / min(50, len(self.experiment_history)),
    ...
    def _learn_success_predictors(
        ...
        for param in successful_params.keys():
            if param in failed_params and len(successful_params[param]) >= 5:
                success_range = (
                    min(successful_params[param]),
                    max(successful_params[param]),
                )
                ...
                if len(failed_params[param]) >= 5:
                    ...
                    if abs(success_mean - failed_mean) > np.std(failed_params[param]):
                        pattern = LearningPattern(
                            ...
                            context={
                                "parameter": param,
                                "optimal_range": success_range,
                                "optimal_mean": success_mean,
                            },
                            last_updated=datetime.now(),
    ...
    def _learn_optimization_strategies(
        self, outcome: ExperimentOutcome
    ) -> List[LearningPattern]:
    ...
    def _detect_failure_patterns(
        ...
        if not outcome.success and len(self.experiment_history) >= 10:
            ...
            recent_failures = [
                exp for exp in self.experiment_history[-20:] if not exp.success
            ]
    ...
    def _learn_convergence_patterns(
        self, outcome: ExperimentOutcome
    ) -> List[LearningPattern]:
    ...
    def _integrate_pattern(self, pattern: LearningPattern):
        ...
        if pattern.pattern_id in self.patterns:
            ...
            existing.confidence = (
                ...
                + pattern.confidence * pattern.observations
            ) / total_obs
            existing.observations = total_obs
            existing.success_rate = (
                existing.success_rate * existing.observations
                + pattern.success_rate * pattern.observations
            ) / total_obs
    ...
    def _calculate_pattern_match(
        self, parameters: Dict[str, float], pattern_context: Dict[str, Any]
    ) -> float:
    ...
    def _find_similar_experiments(
        self, parameters: Dict[str, float], top_k: int = 5
    ...
    def _generate_parameter_variation(
        self, base_params: Dict[str, float], param_space: Dict[str, Tuple]
    ) -> Dict[str, float]:
    ...
    def _generate_pattern_based_recommendations(
        self, objective, param_space: Dict[str, Tuple], num_recommendations: int
    ) -> List[Dict[str, float]]:
        ...
        success_patterns = [
            p for p in self.patterns.values() if p.pattern_type == "success_prediction"
        ]
    ...
    def _adjust_thresholds(self):
        ...
        if len(self.experiment_history) > 50:
            ...
            self.adaptive_thresholds["correlation_threshold"] = max(
                0.3, min(0.8, self.adaptive_thresholds["correlation_threshold"])
            )
    ...
    def _update_performance_metrics(self, campaign_results):
        ...
        self.performance_metrics["prediction_accuracy"] = min(
            1.0, self.performance_metrics["prediction_accuracy"] + 0.05
        )
    ...
    def load_learning_state(self, filepath: Path):
        ...
        try:
            ...
            self.adaptive_thresholds = state.get(
                "adaptive_thresholds", self.adaptive_thresholds
            )
