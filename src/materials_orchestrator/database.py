...
class MongoDatabase(DatabaseInterface):
    ...
    def __init__(
        ...
        database_name: str = "materials_discovery",
    ):
...
def create_database(database_type: str = "file", **kwargs) -> DatabaseInterface:
    ...
    if database_type == "mongodb" and PYMONGO_AVAILABLE:
        ...
        if database_type == "mongodb" and not PYMONGO_AVAILABLE:
            logger.warning(
                "MongoDB requested but pymongo not available, using file database"
            )
...
class ExperimentTracker:
    ...
    def get_successful_experiments(self, campaign_id: str) -> List[ExperimentRecord]:
        ...
        return self.database.query_experiments(
    ...
    def analyze_parameter_performance(
        self, campaign_id: str, target_property: str
    ) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "target_std": (
                sum(
                    (x - sum(target_values) / len(target_values)) ** 2
...
class ExperimentDatabase:
    ...
    def _create_indexes(self):
        ...
        try:
            ...
            experiments.create_index(
                [("campaign_id", ASCENDING), ("timestamp", DESCENDING)]
            )

            # Property-based queries
            experiments.create_index(
                [("results.band_gap", ASCENDING), ("status", ASCENDING)]
            )
    ...
    def store_experiment(self, experiment: Any) -> str:
        ...
        if hasattr(experiment, "to_dict"):
            ...
            doc = (
                asdict(experiment)
                if hasattr(experiment, "__dataclass_fields__")
                else vars(experiment)
            )
        else:
    ...
    def query_experiments(
        ...
        if sort_by:
            for field, direction in reversed(sort_by):
                ...
                results.sort(
                    key=lambda x: self._get_nested_value(x, field) or 0, reverse=reverse
                )
    ...
    def _matches_filter(
        self, doc: Dict[str, Any], filter_criteria: Dict[str, Any]
    ) -> bool:
    ...
    def get_campaign_summary(self, campaign_id: str) -> Optional[Dict[str, Any]]:
        ...
        summary = {
            ...
            "success_rate": len(successful) / len(experiments) if experiments else 0,
            "start_time": min(
                (exp.get("timestamp", "") for exp in experiments), default=""
            ),
            "end_time": max(
                (exp.get("timestamp", "") for exp in experiments), default=""
            ),
