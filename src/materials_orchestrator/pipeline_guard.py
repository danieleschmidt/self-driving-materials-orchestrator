...
class SelfHealingPipelineGuard:
    ...
    def update_health_metric(self, name: str, value: float, threshold: float, severity: str = "warning"):
    ...
    def report_failure(
        ...
        logger.warning(f"Pipeline failure reported: {failure_type.value} in {component}")
            ...
            if self._can_execute_action(action):
                try:
                    ...
                    if success:
                        ...
                        logger.info(f"Successfully healed failure: {failure.failure_id}")
                        break
                    else:
    ...
    async def _execute_healing_action(self, action: HealingAction, failure: PipelineFailure) -> bool:
    ...
    async def _monitor_pipeline_health(self):
        ...
        self.update_health_metric("experiment_success_rate", 100 - experiment_success_rate, 15.0, "warning")
    ...
    def clear_resolved_failures(self):
        ...
        resolved_failures = [
            failure_id for failure_id, failure in self.failures.items()
            if failure.resolved
        ]
