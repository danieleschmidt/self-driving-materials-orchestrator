...
class EnhancedSecurityManager:
    ...
    def validate_request(
        self, request_data: Dict[str, Any], source_id: str, resource: str
    ...
    def _check_rate_limit(self, source_id: str) -> bool:
        ...
        self.rate_limits[source_id] = [
            timestamp
            for timestamp in self.rate_limits[source_id]
        ...
        with self._lock:
            ...
            return {
                ...
                "rate_limited_sources": len(
                    [s for s, times in self.rate_limits.items() if len(times) > 50]
                ),
...
class AdvancedInputValidator:
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                for key, value in data.items():
                    ...
                    if isinstance(value, str):
                        ...
                        if not string_validation.is_valid:
                            issues.append(
                                f"Invalid value for {key}: {string_validation.reason}"
                            )
                    ...
                    # Recursive validation for nested dicts
                        ...
                        risk_score += (
                            nested_validation.risk_score * 0.5
                        )  # Reduce nested impact
                        if not nested_validation.is_valid:
                            issues.append(
                                f"Invalid nested data in {key}: {nested_validation.reason}"
                            )
                        ...
                        for i, item in enumerate(value):
                            if isinstance(item, (str, dict)):
                                ...
                                if not item_validation.is_valid:
                                    issues.append(
                                        f"Invalid list item {i} in {key}: {item_validation.reason}"
                                    )
                ...
                for i, item in enumerate(data):
                    ...
                    if not item_validation.is_valid:
                        issues.append(
                            f"Invalid list item {i}: {item_validation.reason}"
                        )
            ...
            return ValidationResult(
                is_valid=is_valid, reason=reason, risk_score=risk_score
            )

        except Exception as e:
            logger.error(f"Input validation error: {e}")
            return ValidationResult(
                is_valid=False, reason=f"Validation error: {e}", risk_score=1.0
            )
    ...
    def _validate_string(self, text: str) -> ValidationResult:
        ...
        return ValidationResult(is_valid=is_valid, reason=reason, risk_score=risk_score)
...
class CryptoManager:
    ...
    def _is_sensitive_field(self, field_name: str) -> bool:
        ...
        return any(
            pattern in field_name.lower() for pattern in self.sensitive_field_patterns
        )
...
def secure_operation(
    security_level: SecurityLevel = SecurityLevel.INTERNAL, resource_name: str = None
):
